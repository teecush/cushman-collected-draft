...
import { createCatalog } from "./catalog.js?v=200";
import { FEATURES } from "./features.js?v=173";
const DATA_URL = new URL("../site_export/data/catalog.json?v=200", import.meta.url);
...
async function init() {
  try {
    const [response, aliasesResponse, standaloneResponse, curationResponse] = await Promise.all([
      ...
      fetch(new URL('./collection-curation.json?v=200', import.meta.url)),
    ]);
